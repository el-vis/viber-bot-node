...
ViberBot.prototype.broadcastMessage = function(recievers, message, optionalTrackingData) {
	...
	return promise.then(() => new Promise((resolve, reject) => {
		self._broadcastMessage(recievers, message, optionalTrackingData, message.keyboard, message.minApiVersion)
			.then(response => resolve(response), error => reject(error));
	}));
};

ViberBot.prototype._broadcastMessage = function(receivers, message, optionalTrackingData, optionalKeyboard, optionalMinApiVersion) {
	const jsonMessage = message.toJson();
	return this._client.broadcastMessage(receivers, jsonMessage.type, jsonMessage, optionalTrackingData, optionalKeyboard, optionalMinApiVersion);
};
...
ViberBot.prototype._sendMessages = function(userProfile, messages, requestType, optionalTrackingData, optionalChatId) {
	...
	const resolveCallback = (response, message, resolve, reject) => {
		...
		self.emit(EventConsts.MESSAGE_SENT, message, userProfile, response);
		resolve();
	};
	...
	_.each(messages, message => {
		...
		promise = promise.then(() => new Promise((resolve, reject) => {
			if (requestType == REQUEST_TYPE.SEND_MESSAGE)
				return self._sendMessageFromClient(userProfile, message, optionalTrackingData, message.keyboard, optionalChatId, message.minApiVersion)
					.then(response => resolveCallback(response, message, resolve, reject), error => reject(error));
